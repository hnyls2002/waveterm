...
| agents:hookinstalldismissed          | bool     | Set to true to stop Wave from offering to install the Claude Code status-badge hooks into `~/.claude/settings.json` (defaults to false; also set by the "Don't ask again" checkbox on the install prompt)                                                     |
