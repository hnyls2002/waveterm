...
const EmptyState = memo(({ onAddSecret }: { onAddSecret: () => void }) => {
    return (
        <div className="flex flex-col items-center justify-center gap-4 py-12 h-full bg-surface/50 rounded-lg">
            <i className="fa-sharp fa-solid fa-key text-4xl text-muted" />
            <h3 className="text-lg font-semibold text-muted">No Secrets</h3>
...
const CLIInfoBubble = memo(() => {
    return (
        <div className="flex flex-col gap-2 p-4 m-4 bg-surface/50 rounded-lg">
            <div className="flex items-center gap-2">
                <i className="fa-sharp fa-solid fa-terminal text-muted" />
                <div className="text-sm font-medium text-secondary">CLI Access</div>
            </div>
            <div className="font-mono text-xs bg-black/20 px-3 py-2 rounded leading-relaxed text-secondary">
                wsh secret list
                <br />
                wsh secret get [name]
                <br />
            ...
            <div className="flex flex-col divide-y divide-zinc-700">
                {secretNames.map((name) => (
                    <div
                        key={name}
                        className={cn(
                            "flex items-center gap-3 p-4 hover:bg-surfacehover/50 cursor-pointer transition-colors"
                        )}
                ...
                <div
                    className={cn(
                        "flex items-center justify-center gap-2 p-4 hover:bg-surfacehover/50 cursor-pointer transition-colors border-t-2 border-border"
                    )}
                    onClick={onAddSecret}
                >
...
const AddSecretForm = memo(
    ({
        ...
        return (
            <div className="flex flex-col gap-4 min-h-full p-6 bg-surface/50 rounded-lg">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <input
                        type="text"
                        className={cn(
                            "px-3 py-2 bg-surface border rounded focus:outline-none",
                            isNameInvalid
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <textarea
                        className="px-3 py-2 bg-surface border border-border rounded focus:outline-none focus:border-accent-500 font-mono text-sm"
                ...
                <div className="flex gap-2 justify-end">
                    <button
                        className="px-4 py-2 bg-surface hover:bg-surfacehover rounded cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        onClick={onCancel}
                        disabled={isLoading}
                    >
                        Cancel
