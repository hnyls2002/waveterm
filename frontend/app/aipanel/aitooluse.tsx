...
const AIToolApprovalButtons = memo(({ count, onApprove, onDeny }: AIToolApprovalButtonsProps) => {
    ...
    return (
        <div className="mt-2 flex gap-2">
            <button
                onClick={onApprove}
                className="px-3 py-1 border border-border text-secondary hover:border-border hover:text-primary text-sm rounded cursor-pointer transition-colors"
            >
                {approveText}
            </button>
            <button
...
const AIToolUseBatch = memo(({ parts, isStreaming }: AIToolUseBatchProps) => {
    ...
    return (
        <div className="flex items-start gap-2 p-2 rounded bg-surface/60 border border-border">
            <div className="flex-1">
...
const AIToolUse = memo(({ part, isStreaming }: AIToolUseProps) => {
    ...
    return (
        <div
            className={cn("flex flex-col gap-1 p-2 rounded bg-surface/60 border border-border", statusColor)}
                ...
                {isFileWriteTool &&
                    ...
                    Date.now() - toolData.runts < BackupRetentionDays * 24 * 60 * 60 * 1000 && (
                        <button
                            ...
                            className="flex-shrink-0 px-1.5 py-0.5 border border-border hover:border-border hover:bg-surfacehover rounded cursor-pointer transition-colors flex items-center gap-1 text-muted"
...
const AIToolProgress = memo(({ part }: AIToolProgressProps) => {
    ...
    return (
        <div className="flex flex-col gap-1 p-2 rounded bg-surface/60 border border-border">
            <div className="flex items-center gap-2">
