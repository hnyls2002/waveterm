...
const FakeToolCall = memo(({ toolName, toolDescription }: { toolName: string; toolDescription: string }) => {
    return (
        <div className="flex items-start gap-1 p-2 rounded bg-surface border border-border text-success">
            <span className="font-bold">✓</span>
            <div className="flex-1">
                <div className="font-semibold">{toolName}</div>
                <div className="text-sm text-muted">{toolDescription}</div>
            </div>
        </div>
    );
});
...
const FakeUserMessage = memo(({ userPrompt }: { userPrompt: string }) => {
    return (
        <div className="flex justify-end">
            <div className="px-2 py-2 rounded-lg bg-surfacehover text-primary max-w-[calc(100%-20px)]">
    ...
    return (
        <div className="py-2 pl-3 pr-1 border-b border-border flex items-center justify-between min-w-0 bg-panel">
            ...
            <div className="flex items-center flex-shrink-0 whitespace-nowrap">
                ...
                <button
                    className="text-muted transition-colors p-1 rounded flex-shrink-0 ml-2 focus:outline-none"
                    title="More options"
                >
